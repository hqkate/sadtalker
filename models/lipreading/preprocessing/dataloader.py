...
"""
1) doing face detection and face alignment,
2) aligning each frame to a reference mean face shape
...
class AVSRDataLoader(object):
    ...
    def preprocess(self, video_pathname, landmarks_pathname):
        ...
        if (
            ...
            or len(preprocessed_landmarks) < self._window_margin
        ):
            return
    ...
    def crop_patch(self, video_pathname, landmarks):
        ...
        while True:
            ...
            transformed_frame, transformed_landmarks = self.affine_transform(
                frame, smoothed_landmarks, self._reference, grayscale=self._convert_gray
            )
            sequence.append(
                cut_patch(
                    transformed_frame,
        ...
        return (
            np.array(sequence),
            np.array(sequence_frame),
            np.array(sequence_landmarks),
        )
