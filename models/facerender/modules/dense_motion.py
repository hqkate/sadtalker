...
class DenseMotionNetwork(nn.Cell):
    ...
    def __init__(self, block_expansion, num_blocks, max_features, num_kp, feature_channel, reshape_depth, compress,
        ...
        if estimate_occlusion_map:
            self.occlusion = nn.Conv2d(self.hourglass.out_filters*reshape_depth, 1, kernel_size=7, pad_mode='pad', padding=3)
        else:
            self.occlusion = None
    ...
    def create_heatmap_representations(self, feature, kp_driving, kp_source):
        ...
        # adding background feature
        zeros = ops.zeros(heatmap.shape[0], 1, spatial_size[0], spatial_size[1], spatial_size[2]).type(heatmap.type())
