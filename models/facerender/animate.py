...
def load_cpk_facevid2vid(config, generator=None, kp_detector=None, he_estimator=None):
    ...
    generator_path = os.path.join(
        ckpt_dir, config.path.get("generator_checkpoint", None)
    )
    kp_extractor_path = os.path.join(
        ckpt_dir, config.path.get("kp_extractor_checkpoint", None)
    )
...
def load_cpk_mapping(config, mapping):
    ...
    checkpoint_path = os.path.join(
        ckpt_dir, config.path.get("mappingnet_checkpoint", None)
    )
