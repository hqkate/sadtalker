    ...
    while 1:
        ...
        try:
            ...
            for i in tqdm(range(len(images))):
                predictions.extend(
                    detector.detect_faces(np.array(images[i]), return_landm=False)
                )
        except RuntimeError:
            raise RuntimeError(
...
def datagen(frames, mels):
    ...
    for i, m in enumerate(mels):
        ...
        if len(img_batch) >= args.wav2lip_batch_size:
            ...
            img_masked[:, args.img_size // 2 :] = 0.0
...
def main():
    ...
    elif args.face.split(".")[1] in ["jpg", "png", "jpeg", "png"]:
    ...
    if not args.audio.endswith(".wav"):
        ...
        command = "./ffmpeg-6.0-amd64-static/ffmpeg -y -i {} -strict -2 {}".format(
    ...
    command = (
        "./ffmpeg-6.0-amd64-static/ffmpeg -y -i {} -i {} -strict -2 -q:v 1 {}".format(
            args.audio, "results/temp/result.avi", args.outfile
        )
    )
