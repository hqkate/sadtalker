...
sh_a = [
    ms.Tensor(np.pi),
    ms.Tensor(2 * np.pi / np.sqrt(3.0)),
    ms.Tensor(2 * np.pi / np.sqrt(8.0)),
]
sh_c = [
    ms.Tensor(1 / np.sqrt(4 * np.pi)),
    ms.Tensor(np.sqrt(3.0) / np.sqrt(4 * np.pi)),
    ms.Tensor(3 * np.sqrt(5.0) / np.sqrt(12 * np.pi)),
]
...
class ParametricFaceModel:
    def __init__(
        ...
        self.mean_shape = ms.Tensor(model["meanshape"].astype(np.float32))
        # identity basis. [3*N,80]
        self.id_base = ms.Tensor(model["idBase"].astype(np.float32))
        # expression basis. [3*N,64]
        self.exp_base = ms.Tensor(model["exBase"].astype(np.float32))
        # mean face texture. [3*N,1] (0-255)
        self.mean_tex = ms.Tensor(model["meantex"].astype(np.float32))
        # texture basis. [3*N,80]
        self.tex_base = ms.Tensor(model["texBase"].astype(np.float32))
        # face indices for each vertex that lies in. starts from 0. [N,8]
        self.point_buf = ms.Tensor(model["point_buf"].astype(np.int64)) - 1
        # vertex indices for each face. starts from 0. [F,3]
        self.face_buf = ms.Tensor(model["tri"].astype(np.int64)) - 1
        # vertex indices for 68 landmarks. starts from 0. [68,1]
        self.keypoints = list(np.squeeze(model["keypoints"].astype(np.int64)) - 1)

        if is_train:
            # vertex indices for small face region to compute photometric error. starts from 0.
            self.front_mask = (
                ms.Tensor(model["frontmask2_idx"].astype(np.int64)).squeeze() - 1
            )
            # vertex indices for each face from small face region. starts from 0. [f,3]
            self.front_face_buf = ms.Tensor(model["tri_mask2"].astype(np.int64)) - 1
            # vertex indices for pre-defined skin region to compute reflectance loss
            self.skin_mask = ms.Tensor(model["skinmask"]).squeeze()

        if recenter:
            mean_shape = self.mean_shape.reshape([-1, 3])
            mean_shape = mean_shape - ops.mean(mean_shape, axis=0, keep_dims=True)
        ...
        # self.to_camera_mul_arr = ms.Tensor([1., 1., -1.], ms.float32)
        # self.to_camera_add_arr = ms.Tensor([0., 0., 10.], ms.float32)
    ...
    def compute_shape(self, id_coeff, exp_coeff):
        ...
        #     ms.float32
        # )
        exp_part = ops.BatchMatMul(transpose_b=True)(
            ms.Tensor(self.exp_base), exp_coeff
        ).T
        face_shape = (
            id_part + exp_part + ms.Tensor(self.mean_shape.reshape([1, -1]), ms.float32)
        )
    ...
    def compute_texture(self, tex_coeff, normalize=True):
        ...
        face_texture = ops.BatchMatMul(transpose_b=True)(
            ms.Tensor(self.tex_base), tex_coeff
        ).T + ms.Tensor(self.mean_tex, ms.float32)
        if normalize:
    ...
    def compute_color(self, face_texture, face_norm, gamma):
        ...
        y7 = (
            ...
            * self.sh_a[2]
            * self.sh_c[2]
            / ms.Tensor(np.sqrt(3.0))
            * (ms.Tensor(3.0) * face_norm[..., 2:] ** 2 - ms.Tensor(1.0))
        )
    ...
    def to_camera(self, face_shape):
        ...
        # face_shape = face_shape * self.to_camera_mul_arr + self.to_camera_add_arr
    ...
    def compute_for_render_new(self, coeffs):
        ...
        face_norm_roted = ops.matmul(face_norm, rotation)
        face_color = self.compute_color(face_texture, face_norm_roted, gammas)
...
if __name__ == "__main__":
    ...
    landmark = bfm.compute_for_render_landmarks(coeffs)  # (B, 68, 2)
