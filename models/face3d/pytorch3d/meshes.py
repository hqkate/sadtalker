...
class Meshes:
    ...
    def __init__(
        ...
        *,
        ...
        if isinstance(verts, list) and isinstance(faces, list):
        ...
        elif ops.is_tensor(verts) and ops.is_tensor(faces):
            ...
            self.valid = ops.zeros((self._N,), dtype=ms.bool_)
            if self._N > 0:
    ...
    def _set_verts_normals(self, verts_normals) -> None:
        if isinstance(verts_normals, list):
            ...
            for item, n_verts in zip(verts_normals, self._num_verts_per_mesh):
                if (
                    not isinstance(item, ms.Tensor)
                    or item.ndim != 2
                    or item.shape[1] != 3
                    or item.shape[0] != n_verts
                ):
                    raise ValueError("Invalid verts_normals input")
            self._verts_normals_packed = ops.cat(verts_normals, 0)
        elif ops.is_tensor(verts_normals):
            if (
                verts_normals.ndim != 3
                or verts_normals.shape[2] != 3
                or verts_normals.shape[0] != self._N
            ):
                raise ValueError(
                    "Vertex normals tensor has incorrect dimensions.")
    ...
    def __getitem__(
        ...
        if ops.is_tensor(verts) and ops.is_tensor(faces):
    ...
    def verts_padded_to_packed_idx(self):
        ...
        self._verts_padded_to_packed_idx = ops.cat(
            [
                ops.arange(v, dtype=ops.int64) + i * self._V
    ...
    def verts_normals_list(self):
        ...
        if self.isempty():
            return [
                ms.numpy.empty((0, 3), dtype=ops.float32)
            ] * self._N
    ...
    def verts_normals_padded(self):
        ...
        """
        if self.isempty():
            return ops.zeros((self._N, 0, 3), dtype=ms.float32)
    ...
    def faces_normals_list(self):
        ...
        if self.isempty():
            return [
                ms.numpy.empty((0, 3), dtype=ms.float32)
            ] * self._N
    ...
    def _compute_padded(self, refresh: bool = False):
        ...
        """
        if not (
            refresh or any(v is None for v in [
                           self._verts_padded, self._faces_padded])
        ):
            return
    ...
    def _compute_laplacian_packed(self, refresh: bool = False):
        ...
        if self.isempty():
            self._laplacian_packed = ops.zeros(
                (0, 0), dtype=ops.float32
            ).to_sparse()
            return
    ...
    def split(self, split_sizes: list):
        ...
        if not all(isinstance(x, int) for x in split_sizes):
            raise ValueError(
                "Value of split_sizes must be a list of integers.")
        meshlist = []
        curi = 0
...
def join_meshes_as_scene(
    ...
    mesh = Meshes(verts=verts.unsqueeze(
        0), faces=faces.unsqueeze(0), textures=textures)
    return mesh
