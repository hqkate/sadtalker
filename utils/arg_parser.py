...
def _parse_options(opts: list):
    ...
    for opt_str in opts:
        assert (
            "=" in opt_str
        ), "Invalid option {}. A valid option must be in the format of {{key_name}}={{value}}".format(
            opt_str
        )
...
def _merge_options(config, options):
    ...
    for opt in options:
        ...
        hier_keys = opt.split(".")
        assert (
            hier_keys[0] in config
        ), f"Invalid option {opt}. The key {hier_keys[0]} is not in config."
        cur = config[hier_keys[0]]
        for level, key in enumerate(hier_keys[1:]):
            if level == len(hier_keys) - 2:
                assert (
                    key in cur
                ), f"Invalid option {opt}. The key {key} is not in config."
                cur[key] = value
            else:
                assert (
                    key in cur
