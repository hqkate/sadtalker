    ...
    def generate_for_train(self, video_path, save_dir, org_dir, crop_or_resize='crop', source_image_flag=False, pic_size=256):
        ...
        if not os.path.isfile(coeff_path):
            ...
            for idx in tqdm(range(len(frames_pil)), desc='3DMM Extraction In Video:'):
                ...
                im_t = ms.tensor(np.array(im1)/255., dtype=ms.float32).permute(2, 0, 1).unsqueeze(0)
    ...
    def align_256_final(self, input_path, save_dir, org_dir, crop_or_resize='crop', source_image_flag=False, pic_size = 256):
        ...
        if not os.path.isfile(coeff_path):
            ...
            for idx in tqdm(range(len(frames_pil)), desc='3DMM Extraction In Video:'):
                ...
                trans_params = np.array([float(item) for item in np.hsplit(trans_params, 5)]).astype(np.float32)
                im_t = ms.tensor(np.array(im1)/255., dtype=ms.float32).permute(2, 0, 1).to(self.device).unsqueeze(0)
