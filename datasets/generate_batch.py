...
def get_data(
    ...
    img_masked = img.copy()
    img_masked[:, 96 // 2 :] = 0
    img_input = np.concatenate((img_masked, img), axis=3) / 255.0
    first_frame_img = ms.Tensor(np.transpose(img_input, (0, 3, 1, 2)), dtype=ms.float32)
    ...
    # split frames
    T = 12
    indiv_mels = ops.cat(ops.tensor_split(indiv_mels, num_frames // T, axis=1), axis=0)
    ref_coeff = ops.cat(ops.tensor_split(ref_coeff, num_frames // T, axis=1), axis=0)[
        :, :, :64
    ]
    ratio = ops.cat(ops.tensor_split(ratio, num_frames // T, axis=1), axis=0)
